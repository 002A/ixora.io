...
That's how the algorithm works. Besides the above, there are just some bitshifting and bitmasks to encode the 20 bit integers into a 64 bit integer value and then to decode the outcome of the addition of two 64 bit integer values. Have a look at the `code <https://github.com/hx2A/Camera3D/blob/master/src/camera3D/generators/DuboisAnaglyphGenerator64bitLUT.java>`_ on github to see for yourself.
